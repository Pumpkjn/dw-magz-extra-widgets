...
add_action( 'widgets_init', 'dw_mz_category_widgets_init' );
function dw_mz_category_widgets_init() {
	register_widget( 'DW_mz_category_Widget' );
}

class DW_mz_category_Widget extends WP_Widget {
	public function __construct() {
		$widget_ops = array( 'classname' => 'dw_mz_category_widget', 'description' => 'Category Ajax.' );
		parent::__construct( 'dw-category-widget', 'DW MagZ: Category Widget', $widget_ops );
	...
	public function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'dw_mz_category_widget', 'widget' );
		}
		...
		$r = new WP_Query( apply_filters( 'dw_mz_category_widget', $query ) );

		if ( $r->have_posts() ) :
?>
		<?php echo $args['before_widget']; ?>
		
		<?php include DWMZ_DIR . 'templates/category_styles/'. $carousel_style .'.php' ; ?> 
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'dw_mz_category_widget', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['dw_mz_category_widget'] ) ) {
			delete_option( 'dw_mz_category_widget' );
		}
		return $instance;
	}

	public function flush_widget_cache() {
		wp_cache_delete( 'dw_mz_category_widget', 'widget' );
	}
	...
	public function form( $instance ) {
			...
			$category_styles = isset( $instance['category_styles'] ) ? esc_attr( $instance['category_styles'] ) : '';
	...
	?>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'category_styles' ) ); ?>"><?php _e( 'Choose carousel style', 'dw-carousel' ) ?></label>
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'category_styles' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category_styles' ) ); ?>">
				<?php 
				$styles = $this->read_styles();
				for ( $i = 1; $i <= intval( $styles ); $i++) { 
					$style = 'style-'.$i;
					?>
					<option value="<?php echo $style; ?>" <?php selected( $category_styles, $style ) ?>><?php _e( $style , 'dw-carousel' ); ?></option>
					<?php
				}
				?>
			</select></p>		
	...
	<?php
		...
		function read_styles() {
			$dir = DWMZ_DIR . "/templates/category_styles";
			$count = 0;
			$files = scandir( $dir );
